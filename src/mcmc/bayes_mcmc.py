...
def run_mcmc(data,n_walkers,n_steps):
    ...
    initial_wcdm = [67.0,0.2,-0.7,-19.0] + 1e-1*np.random.rand(n_walkers,4)
    with Pool() as pool:
        ...
        samples_lcdm = sampler_lcdm.get_chain(discard=1000, thin=10,flat = True)
    ...
    with Pool() as pool:
        ...
        samples_wcdm = sampler_wcdm.get_chain(discard=1000, thin=10,flat = True)
